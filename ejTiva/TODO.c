...
Descripción:
    ...
    1. Lectura de potenciómetro via ADC (PK3/AIN19)
    ...
    5. Control de motor DC con PWM (PF1 - M0PWM1)
...
uint32_t motorDutyCycle = 850;  // Ciclo de trabajo del motor (50% de 1700)
...
int main(void)
{
    ...
    while(1)
    {
        ...
        // 3. Lectura ADC y control de LEDs/Buzzer
        ...
        DelayMilliseconds(10000); // Pausa entre iteraciones
    }
}
...
void Motor_Init(void)
{
    ...
    PWMGenPeriodSet(PWM0_BASE, PWM_GEN_0, 1700);  // Periodo para ~20kHz
...
uint32_t MeasureDistance(void)
{
    ...
    return (pulseWidth * 0.0343); // / 2); // Convertir a cm
}
